...
export function bootstrapApplication(): void {
    ...
    runHistory.innerHTML = history
      .map((run) => `
        <article class="history-row">
          ...
          <div><span>Result</span><b>${run.resilience} resilience · ${run.accuracy}% accuracy</b><small>${run.completed}/${run.target} addressed · ${formatSeconds(run.elapsedSeconds)} · ${run.difficulty}</small></div>
  ...
  startGameButton.addEventListener("click", () => {
    ...
    game.events.on("mission-started", (runPlan: MissionRunPlan) => {
      runIdentity.innerHTML = `<span>${runPlan.mission.region} · ${runPlan.condition.title}</span><b>Seed ${runPlan.seed} · ${runPlan.activeScenarioIds.length} active disruptions selected</b>`;
    });
