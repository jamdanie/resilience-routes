...
export function renderLandingScreen(): string {
  ...
  const scenarioCount = missionPacks.reduce((total, mission) => total + mission.scenarios.length, 0);
  const ambientCount = missionPacks.reduce((total, mission) => total + mission.ambientEvents.length, 0);
  return `
      ...
      <section class="landing-content">
        <div class="landing-copy">
          ...
          <div class="landing-proof" aria-label="Platform features">
            <span><b>${scenarioCount}</b> decision injects</span>
            <span><b>${ambientCount}</b> temporary injects</span>
            <span><b>Seeded</b> routes and weather</span>
          </div>
        </div>
